...
;------------- Title of Game-------
nameOfGame db " Car Racing $"
toNext2 db " please enter any key to continue...$"

;------------- How to play-------
line1 db " HOW TO PLAY?$"
line2 db " Player 1: navigation W A S D button$"
line3 db " Player 2: navigation UP DOWN RIGHT LEFT button$"
toNext db " please enter any key to continue...$"
story db "Plot: In the Mushroom Kingdom, Mario and Wario had a big race. It was not just fast driving it was also about smart moves.$"
storycontinue db "Mario, in his red kart, faced Wario in his big yellow car. They raced hard. Wario tried to make Mario lose with tricks, but Mario was too good. He dodged all the tricks. In the end, Mario won the race. $"
...
mes5 db  'max size for name is 5 char please enter it again  $'
player1 db 16,?,16 dup('$') ,'$'
player2 db 16,?,16 dup('$') ,'$'

mes6 db  'Choose Track Level (from 1:3) $'
mes7 Db  'INVALID TRACK! $'

; ------------ main Screen variables -------
message0 db 'CAR RACING GAME' , '$'
message1 db '* to Start CHATTING [press F1] $'
message2 db '* to Start playing CAR RACING GAME [press F2] $'
message3 db '* to EXIT the program [press Esc] $'
    

breakLine db '--------------------------------------------------------------------------------$'

sendChat    db '-  YOU SENT CHAT INVITATION TO $'
sendPlay    db '-  YOU SEND A PLAYING INVITATION TO $'
...
; ------------ Chatting mode variables -------
cmes1 db 'CHATTING MODE  $'
...
mov dx,y
mov ax, x_f
mov x_final,ax     ; x_final is considered to be the width of the track
mov ax,y_f
...
;------------------------ Macros ------------------------
showMes macro  mes   ; para sa mga message
    mov ah,09
    lea dx ,mes
    int 21h
endm showMes
...
checkbuffer macro    ;;  al = ascii-Code
...
clearScreen macro
    mov ax ,0600h    ;para ma clear ung screen
    mov bh ,34h
    mov cx,0
    mov dx,184fh
    int 10h
...
x_final dw 0
y_final dw 0
backgroundColor db 34 ;color of trackfield
Game_time dw 0
choose_level_value db ?
...
; Set cursor position and display "nameOfGame"
mov dh, 10  ; pang-baba ng position
mov dl, 35  ; (pang-align to left, center, right)
call setcur2
lea dx, nameOfGame
call outstr2

; Set cursor position and display "toNext2"
mov dh, 16  ; pang-baba ng position
mov dl, 40  ; (pang-align to left, center, right)
call setcur2
lea dx, toNext2
call outstr2
...
; Set cursor position at the bottom left
mov dh, 23  ; Row 23
mov dl, 03  ; Column 3
call setcur2
...
; Set cursor position and display "How to play"
mov dh, 06  ; Row 4
mov dl, 35  ; Column 35 (centered for "How to play" with 11 characters)
call setcur
lea dx, line1
call outstr

; Set cursor position and display "Player 1 navigation"
mov dh, 10  ; pang-baba ng position
mov dl, 22  ; (pang-align to left, center, right)
call setcur
lea dx, line2
call outstr

; Set cursor position and display "Player 2 navigation"
mov dh, 12  ; pang-baba ng position
mov dl, 16  ; (pang-align to left, center, right)
call setcur
lea dx, line3
call outstr

; Set cursor position and display "story"
mov dh, 20  ; pang-baba ng position
mov dl, 0  ; (pang-align to left, center, right)
call setcur
lea dx, story
call outstr

; Set cursor position and display "storycontinue"
mov dh, 22  ; pang-baba ng position
mov dl, 0  ; (pang-align to left, center, right)
call setcur
lea dx, storycontinue
call outstr


; Set cursor position and display "toNext"
mov dh, 16  ; pang-baba ng position
mov dl, 40  ; (pang-align to left, center, right)
call setcur
lea dx, toNext
...
cls proc near
    ...
    mov bh, 34h    ; Attribute (blue background, yellow text)
...
defineNames proc
    ...
    ;; CHECK SIZE
        mov al, player1[1]
        cmp al , 5   ;for size of name of player1
        jbe lbl1
    ...
    ;; CHECK SIZE
        mov al, player2[1]
        cmp al ,5  ;for size of name of player2
        jbe lbl2
...
choose_level proc near 
      ;;   read player 1 name
        ...
        fixLevel:
            clearRow 11 
            setCursor 11,12,0 
            readChar choose_level_value
      ;; check if it 1,2,3
      cmp al, 31h
          mov choose_level_value,1
          jz return
      cmp al,32h
          mov choose_level_value,2
          jz return
      cmp al,33h
          mov choose_level_value,3
          jz return
      notallowedLevel:
        clearRow 12
        setCursor 12,12,0
...
;---------------------- THIS ARE THE LOGIC OF THE GAME-----------------------------------
gameMode proc
    changePage 2 
...
W:
      inc car_two_speed          
      mov al, car_two_speed
      cmp al, car_two_max_speed
      jg fix_5
      jmp mainloop
...
S:
      ...
      fix_5:      
      mov al , car_two_max_speed
      mov car_two_speed , al
      jmp mainloop

      fix_6:      
      mov al , car_two_min_speed
      mov car_two_speed , al
      jmp mainloop
...
;------------- RESTART THE CONFIGURATIONS -----------
	...
	;----------Car Two -----------
    mov ax, 115       ; Set Car Two's X position slightly to the left of Car One
    mov car_two_old_x, ax
    mov car_two_x, ax
    mov ax, 150      ; Set Car Two's Y position the same as Car One's
...
Draw_car_one       PROC    
       ...
       m8:
        ...
        st1:      MOV AH,0Bh        	;set the configuration
              mov ah , 0
              mov al , [DI]
              mov car_one_width , ax
...
Dotted_lineY proc                 ; proc to draw the final line 
	...
	StartDY: 
           ...
           cmp Dx,y_final
	       Jl DrawitDY      	;  check if we can draw current x and y and excape the y iteration
ret
Dotted_lineY endp
;---------------------------------------------------------------------------------
...
mov al,8       ;add 8 to the obstacle to change the color
mov ah,0ch
add ax , obstacle_num
draw1: int 10h
draw_Lenght:int 10h
...
;------------- Set Score --------------
set_score proc
    cmp al, 41        ; Compare the ASCII value with 'A'
    jl not_valid      ; If less than 'A', it's not valid
    cmp al, 46        ; Compare the ASCII value with 'F'
    jg not_valid      ; If greater than 'F', it's not valid

    cmp al, 48        ; Compare the ASCII value with '0'
    jge done          ; If greater than or equal to '0', it's a valid digit
    mov al, 48        ; Otherwise, set to '0'
    ret               ; Return
...
who_win proc
    ...
    ; Compare scores to determine winner or draw
    mov al, Score_CarOne
    mov ah, Score_CarTwo
    cmp al, ah
    jg playerOne   ; If CarOne's score is greater, go to playerOne
    jl playerTwo   ; If CarTwo's score is greater, go to playerTwo
...
playerOne:
    ; Display player one's name and winner message
    showMes player1[2]
    setCursor 12, 17, 0
    showMes winner_message
    ret

playerTwo:
    ; Display player two's name and winner message
    showMes player2[2]
    setCursor 12, 17, 0
    showMes winner_message
    ret
